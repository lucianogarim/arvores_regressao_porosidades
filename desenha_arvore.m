function desenha_arvore(tree,titletext)
...
for i = 1:length(tree.filhos)
    [tree.filhos{i}, state] = walkTreeDepthFirst(tree.filhos{i}, preDive, postDive, initialDepth + 1, state);
end
if ~isempty(postDive)
...
%     text(tree.X, tree.Y, opText, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'top', 'interpreter', 'none')
    text(tree.X, tree.Y, num2str(tree.classe) ,'HorizontalAlignment', 'right', 'VerticalAlignment', 'top', 'interpreter', 'none');
end
